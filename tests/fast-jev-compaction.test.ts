...
describe('keep threshold', () => {
  ...
  it('drops only when Jev is confident the call is spent', () => {
    ...
    // Split gates: the call is dropped only below keepCallThreshold (0.05),
    // which is stricter than the result's keepThreshold (0.15). The intent of
    // this assertion is unchanged; the input moves to the same side of the
    // call's own gate.
    expect(decideCall(call, { keepCall: 0.01, keepResult: 0.03 }, options).action).toBe(
      'drop_call',
    );
  });
});
